...
def manhattan_distance(a, b): #Manhattan distance
...
def astar(maze, start, end):
    ...
    runtime = (end_time - start_time) * 1000  # Convert to milliseconds
    return -1, "NULL", nodes_created, runtime # Return the path
...
def main():
    ...
    for i, case in enumerate(test_cases):# Loop through test cases
        ...
        cost, path, nodes_created, runtime = astar(case["maze"], case["start"], case["goal"]) # Call astar function
        print(f"1) Cost of the path found: {cost}") # Print path cost
        print(f"2) Path: {path}") # Print path
        print(f"3) Number of nodes created: {nodes_created}")# Print number of nodes created
